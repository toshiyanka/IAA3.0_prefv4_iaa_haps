...
package hqm_pkg ;
    ...
    // COMP 
    // COMP_T
    // RELEASE is now deprecated in V3
    // ARM
    // A_COMP
    // A_COMP_T
    // NEW 
    ...
    //
    //
    typedef enum logic [3:0] { HQM_CMD_NOOP = 4'b0000, HQM_CMD_BAT_TOK_RET = 4'b0001, HQM_CMD_COMP = 4'b0010, HQM_CMD_COMP_TOK_RET = 4'b0011, HQM_CMD_ILLEGAL_CMD_4 = 4'b0100, HQM_CMD_ARM = 4'b0101, HQM_CMD_A_COMP = 4'b0110, HQM_CMD_A_COMP_TOK_RET = 4'b0111, HQM_CMD_ENQ_NEW = 4'b1000, HQM_CMD_ENQ_NEW_TOK_RET = 4'b1001, HQM_CMD_ENQ_COMP = 4'b1010, HQM_CMD_ENQ_COMP_TOK_RET = 4'b1011, HQM_CMD_ENQ_FRAG = 4'b1100, HQM_CMD_ENQ_FRAG_TOK_RET = 4'b1101, HQM_CMD_ILLEGAL_CMD_14 = 4'b1110, 
            HQM_CMD_ILLEGAL_CMD_15 = 4'b1111} hcw_cmd_dec_t ; 
    ...
    typedef enum logic [1:0] { ATOMIC = 2'h0, UNORDERED = 2'h1, ORDERED = 2'h2, DIRECT = 2'h3} qtype_t ; 
    typedef struct packed {
        logic [2:0] msgtype ; 
        logic [2:0] qpri ; 
        qtype_t qtype ; 
    ...
    // NEW: from system per-qid storage to instruct CHP to add 16b TS delta to HCW
    typedef struct packed {
        logic parity ; 
        logic [4:0] // covers vas[4:0] , pp_is_ldb, pp[7:0], qe_is_ldb, qid[7:0]
            vas ; 
        logic pp_is_ldb ; 
        logic [7:0] pp ; 
        logic qe_is_ldb ; 
    ...
    typedef struct packed {
        ...
        logic // parity[1] covers data[127:64], parity[0] covers data[63:0]
            parity ; 
        logic // covers all wuser fields except for hcw_parity
            ao_v ; 
        logic [4:0] vas ; 
        logic pp_is_ldb ; 
        logic [7:0] pp ; 
    ...
    typedef struct packed {
        logic parity ; 
        logic pad_ok ; 
        logic //    LSP-to-CHP-to-SYSTEM: the HCW has a qtype UNO,ORD or ATM (1) or DIR (0) 
            cq_is_ldb ; 
        logic [(2 - 1):0] //    LSP-to-CHP: 2 bit Congestion Management Mechanism flag.
            // 11:                            depth >      threshold
            // 10: depth <=     threshold  &  depth >  .75 threshold
            // 01: depth <= .75 threshold  &  depth >  .50 threshold
            // 00:                            depth <= .50 threshold
            congestion_management ; 
        logic [(2 - 1):0] //    LSP-to-CHP-to-WB: Write Buffer Optimization.
            ...
            write_buffer_optimization ; 
        logic //    LSP-to-CHP: scheduled CQ while ignore_depth config bit was set
            ignore_cq_depth ; 
    } hqm_core_flags_t ; 
    //this is the CQ assoicated with the HCW data
    typedef struct packed {
    ...
    typedef struct packed {
        ...
        logic [(HCW_SCHED_ADDR_WIDTH - 1):0] //NOTUSED
            addr ; 
        hcw_sched_aw_req_awuser_t //NOTUSED
            user ; 
    } hcw_sched_aw_req_t ; 
    localparam BITS_HCW_SCHED_AW_REQ_T = $bits(hcw_sched_aw_req_t) ; 
    // parity[1] covers data[127:64], parity[0] covers data[63:0]
    typedef struct packed {
        hqm_core_flags_t hqm_core_flags ; 
        logic [12:0] cq_wptr ; 
        logic [7:0] cq ; 
        logic [1:0] //this is the CQ assoicated with the HCW data
            hcw_parity ; 
    ...
    //this is the CQ associated with the cq occupancy interrupt
    typedef struct packed {
        logic parity ; 
        logic is_ldb ; 
        logic [(7 - 1):0] cq_occ_cq ; 
    } interrupt_w_req_t ; 
    ...
    typedef enum logic [(2 - 1):0] { PM_FSM_D3COLD, PM_FSM_D3HOT, PM_FSM_D0UNINIT, PM_FSM_D0ACT} pm_fsm_t ; 
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    parameter NUM_RAMS_CORE = ((((((((NUM_RAMS_AQED + NUM_RAMS_AP) + NUM_RAMS_CHP) + NUM_RAMS_DP) + NUM_RAMS_LSP) + NUM_RAMS_NALB) + NUM_RAMS_QED) + NUM_RAMS_ROP) + NUM_RAMS_SYSTEM) ; 
    ...
    parameter NUM_SCHAINS_CORE = ((((((NUM_SCHAINS_AQED + NUM_SCHAINS_CHP) + NUM_SCHAINS_DP) + NUM_SCHAINS_LSP) + NUM_SCHAINS_NALB) + NUM_SCHAINS_QED) + NUM_SCHAINS_ROP) ; 
