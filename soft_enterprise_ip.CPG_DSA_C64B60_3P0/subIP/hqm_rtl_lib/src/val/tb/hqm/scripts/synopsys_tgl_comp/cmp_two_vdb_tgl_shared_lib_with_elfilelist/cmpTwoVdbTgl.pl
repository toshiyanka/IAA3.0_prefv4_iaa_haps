...
sub gen_verdi_tcl {
    ...
    if( $Has_El_File_List =~ "TRUE" ) {
        print FP "-elfile_list $El_File_List ";
    }
...
sub parse_argv {
  for(my $i=0; $i<=$#ARGV; $i++) {
    ...
    elsif( $ARGV[$i] =~ /^-elfile_list/i ) {
      $i++;
      $El_File_List = $ARGV[$i];
      $Has_El_File_List = "TRUE";
    }
