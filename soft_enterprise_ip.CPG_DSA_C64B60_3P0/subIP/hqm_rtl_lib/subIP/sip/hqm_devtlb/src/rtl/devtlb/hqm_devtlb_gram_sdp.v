...
//
`ifndef HQM_DEVTLB_GRAM_SDP_V
`define HQM_DEVTLB_GRAM_SDP_V


`include "hqm_devtlb_gram_sdp.vh"

module hqm_devtlb_gram_sdp (
//lintra -68099
...
parameter GRAM_STYLE     =`HQM_GRAM_AUTO; // GRAM_AUTO, GRAM_BLCK, GRAM_DIST

input  logic                         clk;
input  logic                         we;
input  logic [BUS_SIZE_ADDR-1:0]     waddr;
input  logic [BUS_SIZE_DATA-1:0]     din;
input  logic [BUS_SIZE_ADDR-1:0]     raddr;
output       [BUS_SIZE_DATA-1:0]     dout;

`ifdef HQM_DEVTLB_SIMONLY
(* `HQM_GRAM_STYLE = GRAM_STYLE *)
output reg [BUS_SIZE_DATA-1:0] ram [(2**BUS_SIZE_ADDR)-1:0];
`else
  ...
  case (GRAM_MODE)
    ...
    //-----------------------------------------------------------------------
         ...
         begin                                  /* synthesis translate_off */
                if(driveX)
                        dout <= {$bits(dout){1'bx}}; //lintra s-50002
    ...
    //-----------------------------------------------------------------------
         ...
         begin
                ram_dout = ram[raddr];
                                                /*synthesis translate_off */
                if((raddr==waddr) && we)
                ram_dout = {$bits(ram_dout){1'bx}};                 /*synthesis translate_on */
         end
         always @(posedge clk)
         begin
                dout <= ram_dout;
