...
EOF
    ...
    if (-d "/p/$ENV{PROJECT}/val/release/$model_path" or 
        ...
        -d "/p/$ENV{PROJECT}/val/release/$clt/${model_path2}") {
        if ((abs_path($ENV{IP_ROOT}) eq abs_path("/p/$ENV{PROJECT}/val/release/$model_path")) or
            ...
            (abs_path($ENV{IP_ROOT}) eq abs_path("/p/$ENV{PROJECT}/val/release/$clt/${model_path2}")) or		## ylai4 - to accommodate sub-clt e.g. sbi
...
echo "Hacking pcov file for $cur_scope!!!"
echo "Pcov file : $testrun_path/$pcov_name.pcov.gz"
perl -I\$IP_ROOT/ace/lib/common -e 'use common; &hack_pcov_file(-path => "$testrun_path", -file => "${pcov_name}.pcov.gz", -original_text => "FLASHCLT_ENT.SVCOV_TB", -replace_text => "flashclt_ent.svcov_tb")'
perl -I\$IP_ROOT/ace/lib/common -e 'use common; &hack_pcov_file(-path => "$testrun_path", -file => "${pcov_name}.pcov.gz", -original_text => "FLASHCLT_ENT.FLASH0", -replace_text => "flashclt_ent.flash0")'
