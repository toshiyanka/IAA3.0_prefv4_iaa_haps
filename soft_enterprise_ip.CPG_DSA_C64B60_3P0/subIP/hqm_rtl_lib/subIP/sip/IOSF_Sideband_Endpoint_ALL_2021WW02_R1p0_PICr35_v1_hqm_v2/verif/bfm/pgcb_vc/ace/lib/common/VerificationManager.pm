...
sub create_scrag {
  ...
  my $debug       = "$testrun_path/sv2c_${pcov_name}";
  ...
  my $model_temp = $ENV{MODEL_ROOT};
     $model_temp =~ s/\/$//;
  my $model;
  if (-l $model_temp){
    $model = basename abs_path($model_temp);
  } else {
    $model = $ENV{ACE_PWA_DIR};
  }
...
# --------------------------------
if [ ! -f "$converter" ]; then
  echo "ERROR: Verification Manager: $converter doesn't exist. Please check with your DA!"
  checkrc CONVERT_COVERAGE 1 
  COV_CONV_STATUS='FAIL'
  exit 1
fi

if [ "\$cdb_switch" ] ; then

  echo "VerificationManager: Generating PCOV file..."
  echo " ($converter $conv_opts $pcov_name $elab_vdb \$cdb_switch) 2>&1 | tee -a $debug"
  ($converter $conv_opts $pcov_name $elab_vdb \$cdb_switch) 2>&1 | tee -a $debug

  if [ -f "$debug" ] ; then
    result=`grep "sv2c-W\\|sv2c-E" $debug` 
    if [ "\$result" ] ; then
      echo "ERROR: Verification Manager: SV2Coral failed to generate pcov file!"
      checkrc CONVERT_COVERAGE 1 
      COV_CONV_STATUS='FAIL'
      exit 1
    else
      echo "SV2Coral successful!"
      rm -f $debug
      checkrc CONVERT_COVERAGE 0
      COV_CONV_STATUS='PASS'
    fi
  else 
    echo "ERROR: Verification Manager: SV2Coral failed to create log file!"
    checkrc CONVERT_COVERAGE 1 
    COV_CONV_STATUS='FAIL'
    exit 1
  fi  
...
#------------------------------------  
      ...
      if (!$released_model) {
        ...
        my $tmp_dir = $testrun_path . "/$ww/$model/$cfg/$pcov_name";
        my $dir = $base_dir . "/$ww/$model/$cfg/$pcov_name";
...
## ylai4 - check if current machine matching with cluster home-site, if same then no need to go through rsync server, thus to reduce rsync server loads
        ...
        if ($current_site eq $home_site) {
          ...
          $fragment .= "chmod -Rf 770 $sync_path/$ww\n";
        }
        else {
