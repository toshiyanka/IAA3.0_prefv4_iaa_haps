...
my $synplify_premier="";
$synplify_premier=  ef_lib::which_synplify_premier();

my @args = @ARGV;
our  $debug = 0; #To determine the DEBUG messages to display
our $log = $ENV{'EMUL_RESULTS_DIR'}."/DISCOVER_FPGA/discover_fpga.log"; 
open (LOG, ">$main::log") || die "Could not create log file $main::log. $!\n"; 
close LOG;

ef_lib::printl("DISCOVER_FPGA : EMUL_MAX_JOB_CNT $EMUL_MAX_JOB_CNT\n",$main::log);
...
ef_lib::printl("DISCOVER_FPGA : METADATA file $metadata_file\n",$main::log);
ef_lib::printl("DISCOVER_FPGA : Top module $top_module\n",$main::log);
ef_lib::printl("DISCOVER_FPGA : reconfig file $reconfig_file\n",$main::log);
...
our $jobkillscript = $ENV{'EMUL_RESULTS_DIR'}."/DISCOVER_FPGA/jobkillscript.tcsh";

our $prj_dir = $ENV{'EMUL_RESULTS_DIR'}."/DISCOVER_FPGA/projects/";
...
if (defined($main::reconfig{'-config_opt'})==1){
   ...
   if (defined($main::config_opt->{'-restart'})==1){
      ...
      &ef_lib::printl("LOG : -config_opt -restart  $main::restart_filename.\n",$main::log);
   }
   if (defined($main::config_opt->{'-restart_id'})==1){
      $main::restart_id =  $main::config_opt->{'-restart_id'};
      &ef_lib::printl("LOG : -config_opt -restart_id  $main::restart_id.\n",$main::log);
   }
   if (defined($main::config_opt->{'-write_reconfig_proto'})==1){
      ...
      &ef_lib::printl("LOG : -config_opt -write_reconfig_proto  $main::reconfig_proto_filename.\n",$main::log);
   }
   if (defined($main::config_opt->{'-write_discover_proto'})==1){
      ...
      &ef_lib::printl("LOG : -config_opt -write_discover_proto  $main::discover_proto_filename.\n",$main::log);
   }
...
if ($main::restart_id<=1){
   &main::PrintModuleLists($prj_dir);
   &main::PrintExitStatus();
   if ($main::discover_proto_enable){
      &main::DumpDiscoveredInfo($main::all_projects_info, $main::discover_proto_filename);
   }
   &fpga_flow::dump_hash("restart", $main::all_projects_info, $ENV{'EMUL_RESULTS_DIR'}."/STATE1.pm");
}


if ($main::restart_id==2){
...
if ($main::restart_id<=2){
   ...
   &fpga_flow::dump_hash("restart", $main::all_projects_info, $ENV{'EMUL_RESULTS_DIR'}."/STATE2.pm");
}
...
sub PrintModuleLists($){
   my $prj_dir = shift;
   my @lib_list= keys %{$all_projects_info};;
   my $ModuleListFile;
   foreach my $lib_name (@lib_list){
      my $project_info  = $all_projects_info->{$lib_name};
      $ModuleListFile = $prj_dir."/".$lib_name."_Modules.lst";
      open (MLST, ">$ModuleListFile") || die "Could not create module list file $ModuleListFile. $!\n"; 
      $project_info = $all_projects_info->{$lib_name};
      if (defined($project_info->{'-non_empty_project'}) == 1){
         if (defined(%{$project_info->{'-FoundTopLevels'}})==1){
            ef_lib::printl("LOG : Module list for $lib_name\n", $main::log);
            print MLST "Module list for $lib_name\n";
...
#                  ef_lib::printl("LOG : \t\t$param : $value\n", $main::log);
                  print MLST "  \t\t$param : $value\n";
               }
            }
         }
         if (defined(%{$project_info->{'-FoundTopLevels_ErrorStatus'}})==1){
            foreach my $Error_Status (keys %{$project_info->{'-FoundTopLevels_ErrorStatus'}}) {
               ef_lib::printl("LOG : Module list for $lib_name error status $Error_Status :\n", $main::log);
               print MLST "Module list for $lib_name error status $Error_Status :\n";
               foreach my $module (@{$project_info->{'-FoundTopLevels_ErrorStatus'}{$Error_Status}}){
                  ef_lib::printl("LOG : \t\t$module\n", $main::log);
                  print MLST " \t\t$module\n";
               }
            }
...
sub PrintExitStatus(){
   ...
   foreach my  $lib_name (@lib_list){
      ...
      $project_info = $all_projects_info->{$lib_name};
      if (defined($project_info->{'-non_empty_project'}) == 1){
         if (defined($project_info->{'-exit_status'})==1){
            my $exit_status = $project_info->{'-exit_status'};
            push(@{$StatusRef->{$exit_status}}, $lib_name);
         }
      }
   }
   ef_lib::printl("LOG : Exit status report: \n", $main::log);
   foreach my $status (keys %{$StatusRef}){
      my $status_str = $synplicity::status_code{$status};
      ef_lib::printl("LOG : \t Exit status : $status/$status_str\n", $main::log);
      foreach my $name (@{$StatusRef->{$status}}){
         ef_lib::printl("LOG : \t\t$name\n", $main::log);
      }
   }
}
...
sub BuildToplevel($$$$$){
   ...
   my $prj_link_def = [];
#   $prj_dir = $ENV{'EMUL_RESULTS_DIR'}."/DISCOVER_FPGA/projects";
...
#         CreateSynplifyProject($lib_name, $prj_impl, $prj_Src_filename, $prj_OptsPaths_filename, $prj_content, $prj_param_def, $prj_link_def);
         ...
         push (@{$prj_cmds}, 'set single_module_only 0'."\n");
         ...
         $project_info->{'-NB_run'}{'-toplevel_detect'}{'-job_cnt'} = $lib_cnt;
         $project_info->{'-NB_run'}{'-toplevel_detect'}{'-job_number'} = $lib_number;
   ...
   foreach $lib_name (@lib_list){
      ...
      if (defined($project_info->{'-non_empty_project'}) == 1){
         ...
         foreach my $line (@{$project_info->{'-NB_run'}{'-toplevel_detect'}{'-job_log'}}){
            ...
            if ( ($ReadTopList==0 ) && ($line =~ /exit status.*/) ){
               my $exit_status = $line;
               chomp $exit_status;
               $exit_status =~ s/exit status=(\d+)/$1/;
               $project_info->{'-exit_status'} = $exit_status;
               $project_info->{'-NB_run'}{'-toplevel_detect'}{'-exit_status'} = $exit_status;
               ef_lib::printl("LOG : exit_status $exit_status\n", $main::log);
               if ($exit_status == 25){
                  $ReadTopList=1;
#                  my $TopmoduleListFileName = $ENV{'EMUL_RESULTS_DIR'}.'/DISCOVER_FPGA/projects/'.$lib_name.'_TopLevelDetect.lst';
...
#                  foreach my $module_discr (@{$lst_arr}){
                  ...
                  if (($module_discr =~ /.+\..+\s+/)==1){
                     ...
                     ef_lib::printl("LOG : FoundTopLevels  $module_name\n", $main::log);
                  }
#                  }
               }elsif ($exit_status == 2){ #if ($exit_status == 25){
                  $ReadTopList=1;
                  my $tbl_arr = [];
                  # Read verilog.tbl, extract 
                  my $VerilogTableFileName = $prj_dir.$lib_name.'_TopLevelDetect/synwork/verilog.tbl';
                  if (-e $VerilogTableFileName){
                     open (LST, "<$VerilogTableFileName") || ef_lib::mydie ("Could not open file $VerilogTableFileName $!\n", $main::log);
                     @{$tbl_arr} = <LST>;
                     close(LST);
                  }else{
                     @{$tbl_arr} = [];
                     ef_lib::printl("LOG : missing Verilog table file $VerilogTableFileName\n", $main::log);
                  }
                  # Read vhdl.tbl, extract 
                  my $VHDLTableFileName = $prj_dir.$lib_name.'_TopLevelDetect/synwork/vhdl.tbl';
                  if (-e $VHDLTableFileName){
                     open (LST, "<$VHDLTableFileName") || ef_lib::mydie ("Could not open file $VHDLTableFileName $!\n", $main::log);
                     @{$tbl_arr} = (@{$tbl_arr}, <LST>);
                     close(LST);
                  }else{
                     @{$tbl_arr} = [];
                     ef_lib::printl("LOG : missing VHDL table file $VHDLTableFileName\n", $main::log);
                  }
                  my $ModuleTableSize = scalar(@{$tbl_arr});
                  ef_lib::printl("LOG : Module table lenght $ModuleTableSize\n", $main::log);
                  foreach my $table_line (@{$tbl_arr}){
                     chomp($table_line);
                     # |upl_cppxpunit|cppxpmipiunit_wrapper|0|/nfs/fm/disks/fm_sptlp_00078/rmeessen/project/pcie/pcie3_2013WW39_R1p0_v3/SPTLP_2013WW39_R1p0_v3_1209/subIP/FIA/source/rtl/fia/cppxpunit/cppxpmipiunit_wrapper.v|0|
                     my $module_name;
                     my @tbl_arr;
                     (@tbl_arr) = split(/\|/, $table_line);
                     if (scalar(@tbl_arr)==6){
                        if ($tbl_arr[1] eq $lib_name){
                           my $tbl_module_id   = $tbl_arr[2];
                           push(@{$project_info->{'-FoundTopLevels_ErrorStatus'}{$exit_status}}, $tbl_module_id);
                           ef_lib::printl("LOG : FoundTopLevels_ErrorStatus $exit_status table entry  $tbl_module_id\n", $main::log);
                        }
                     }else{
                        ef_lib::printl("LOG : BAD table entry for lib : $lib_name >$table_line<\n", $main::log);
                     }
                  }
               }else{ #}elsif ($exit_status == 2){
...
sub NBEnque($$$$){
   ...
   my $job_cnt = $job_info->{'-job_cnt'};
   my $job_number = $job_info->{'-job_number'};
...
#      Your job has been queued (JobID 1485860309, Class SUSE, Queue fm_vp, Slot /CSG/All/SIP/psf)
      ...
      if ($results->[0] =~ /^Your job has been queued.*/){
         ...
         chomp $JobID;
         ef_lib::printl(sprintf("LOG : Submitting netbatch job : $JobID/$JobCmd $job_cnt of $job_number\n"), $main::log);
      ...
      ef_lib::printl(sprintf("LOG : Start interactive job : $job_cnt of $job_number $JobCmd\n"), $main::log);
      @{$results} = `$JobCmd`;
...
sub DumpDiscoveredInfo($$){
   ...
   foreach my $lib_name (keys %{$all_projects_info}){
      my $TopModuleName;
      my @FoundTopLevels = keys %{$all_projects_info->{$lib_name}{'-FoundTopLevels'}};
#       if (defined($all_projects_info->{$lib_name}{'-FoundTopLevels'})==1){
#          my @FoundTopLevels = keys %{$all_projects_info->{$lib_name}{'-FoundTopLevels'}};
      $tmp_hash{$lib_name}{'-ignore_lib'} = 1;
      if (scalar(@FoundTopLevels)>0){
         $TopModuleName = $FoundTopLevels[0];
         $TopModuleName=~ s/.*\.(\S+)/$1/;
         push (@{$tmp_hash{$lib_name}{'-top'}},  $TopModuleName);
         $tmp_hash{$lib_name}{'-ignore_lib'} = 0;
      }else{
         if (defined($all_projects_info->{$lib_name}{'-FoundTopLevels_ErrorStatus'})==1)  {
            if (defined($all_projects_info->{$lib_name}{'-FoundTopLevels_ErrorStatus'}{2})==1){
               $TopModuleName = $all_projects_info->{$lib_name}{'-FoundTopLevels_ErrorStatus'}{2}[0];
               $TopModuleName=~ s/.*\.(\S+)/$1/;
               push (@{$tmp_hash{$lib_name}{'-top'}},  $TopModuleName);
               $tmp_hash{$lib_name}{'-ignore_lib'} = 0;
            }elsif (defined($all_projects_info->{$lib_name}{'-FoundTopLevels_ErrorStatus'}{25})==1){
               $TopModuleName = $all_projects_info->{$lib_name}{'-FoundTopLevels_ErrorStatus'}{25}[0];
               $TopModuleName=~ s/.*\.(\S+)/$1/;
               push (@{$tmp_hash{$lib_name}{'-top'}},  $TopModuleName);
               $tmp_hash{$lib_name}{'-ignore_lib'} = 0;
            }
