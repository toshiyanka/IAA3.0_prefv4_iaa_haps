...
if { [file exists ./outputs/SMD_dblsync.tcl]} {
   sh rm ./outputs/SMD_dblsync.tcl
}
  ...
  if { [sizeof_collection $mytp] > 0 } {
    ...
    foreach_in_collection p $mytp { 
      ...
      if { $mystartclk != "" } {
        ...
        echo "set_max_delay $myfastclk_period -from \[get_clocks  $mystartclk \] -through \[get_pins [get_object_name $dpin] \] " >> ./outputs/SMD_dblsync.tcl
      }
    }
  }
}
