...
BEGIN {#Code in begin block must run first to make it available for the "use lib" call. 
    ...
    my $use_unofficial_contour = 0;#Shouldn't do our turnins with a work-based contour. 

    if ($use_unofficial_contour) {
        #If you need to test with a customized contour (usually when there are cross-IP dependencies, so you must test this VC with another pre-release VC)
        # You may use the CONTOUR_OVERRIDE to point at a clone of the contour which you have overriden locally.
        # The other method is to use individual overrides of the VCs IPToolData.pm.
        my $CONTOUR_OVERRIDE = "/nfs/sc/disks/sc_spi_00016/VC_WORK_DISK/vc_contour-chs-a0";
        $ToolConfig_tools{vc_contour}{VERSION} = "WORK";
        $ToolConfig_tools{vc_contour}{PATH} = "$CONTOUR_OVERRIDE";
    } else {
        my $VC_CONTOUR_RELEASE_AREA = "$RTL_CAD_ROOT/val_ip/vc_contour";
        $ToolConfig_tools{vc_contour}{VERSION} = "1.0.18ww22a";
