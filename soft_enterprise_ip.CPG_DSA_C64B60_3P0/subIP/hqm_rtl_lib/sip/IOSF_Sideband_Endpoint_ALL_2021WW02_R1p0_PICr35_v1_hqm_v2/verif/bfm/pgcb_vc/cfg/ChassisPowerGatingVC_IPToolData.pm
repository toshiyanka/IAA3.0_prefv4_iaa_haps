...
my $dirname;
my $version;
if (defined $ToolData::ToolConfig_tools{ipconfig}{SUB_TOOLS}{ChassisPowerGatingVC}{VERSION} && 
            $ToolData::ToolConfig_tools{ipconfig}{OTHER}{OVERRIDES_FIRST}) {
    $version = $ToolData::ToolConfig_tools{ipconfig}{SUB_TOOLS}{ChassisPowerGatingVC}{VERSION};
    $dirname = $ToolData::ToolConfig_tools{ipconfig}{SUB_TOOLS}{ChassisPowerGatingVC}{PATH};
} else {
...
$ToolConfig_ips{ChassisPowerGatingVC} = {   
    PATH  => $dirname, 
    VERSION => $version, 
    ENV     => {
#        ACE_RC => "$dirname/cfg/ace/ChassisPowerGatingVC.acerc",
    },
    OTHER   => {
        ...
        SUB_SCOPES => [qw(ovm uvm)],
        ## XHDK74 <scope>_IPToolData vars 
        ip_name => 'ChassisPowerGatingVC', ## name expected to be in the ipconfig hash 
        DO_NOT_ALLOW_UNIQUIFY => 1, ## IPs which must ALWAYS be global, such as ovm
        required_env_vars => "&get_env_var()",
        ti_env_pkgs => [ 'ChassisPowerGatingVC_val_lib' ], ## these are the val packages that are provided (especially what the customer is intended to import)
        ...
        ace => {
            IMPORT_INTO_SIP_SHARED_LIB => 'ChassisPowerGatingVC_val_lib', ## package name expected to be imported/present in sip_shared_lib
            reuse_search_paths => ["&get_tool_var(,SEARCH_PATHS)"],
            integ_udf_file => 'cfg/ace/ChassisPowerGatingVC/ChassisPowerGatingVC_integ.udf',
            hdl_udf_file => 'cfg/ace/ChassisPowerGatingVC/ChassisPowerGatingVC_hdl.udf',
            export_model => 'ChassisPowerGatingVC', ## name of the model that ovm is providing with export section
            reuse_lib_paths => [], ## does this map to SDG LIB_PATH?
            subscopes => ["&get_tool_var(,SUB_SCOPES)"],
        },
        library => { val => { lib_name => 'ChassisPowerGatingVC_val_lib', }, }, 
    },
};

#Below code for compatibility with TGL style ToolConfig_ips
if (!defined $ToolData::ToolConfig_tools{ipconfig}{SUB_TOOLS}{ChassisPowerGatingVC}) {
    $ToolConfig_tools{ipconfig}{SUB_TOOLS}{ChassisPowerGatingVC}{PATH} = $ToolConfig_ips{ChassisPowerGatingVC}{PATH};
    $ToolConfig_tools{ipconfig}{SUB_TOOLS}{ChassisPowerGatingVC}{VERSION} = $ToolConfig_ips{ChassisPowerGatingVC}{VERSION};
    $ToolConfig_tools{ipconfig}{SUB_TOOLS}{ChassisPowerGatingVC}{ENV} = $ToolConfig_ips{ChassisPowerGatingVC}{ENV};
}
