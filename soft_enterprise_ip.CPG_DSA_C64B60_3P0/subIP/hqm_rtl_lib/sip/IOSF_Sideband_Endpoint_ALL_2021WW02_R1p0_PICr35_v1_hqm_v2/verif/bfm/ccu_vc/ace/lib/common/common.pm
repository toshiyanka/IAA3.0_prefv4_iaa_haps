...
sub hashCFG {
    my $run_dir = shift @_;
    local(%rscfg);
    print "HASHCFG: $run_dir/runsim.cfg\n";
    open(CONFIG_FILE, "$run_dir/runsim.cfg") || print "\nHASHCFG: CANNOT FIND CFG FILE!\n\n";
    while (<CONFIG_FILE>) {
     ## put in a check for multiline lines
     while( $_ =~ /\\\s*$/ ) {
     ...
     s/^\s+//; s/\s+$//;      # remove leading & trailing spaces
     @temp = split(/\s+/,$_,2);
     $key  = shift @temp;     # get the key, the rest is args
     $key  =~ s/\://o;
     $rscfg{$key} = shift @temp;   # Assumes one arg per tag
    }
    close(CONFIG_FILE);
    return(%rscfg);
}
