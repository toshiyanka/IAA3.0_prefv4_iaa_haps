// $Header: /ds/data/2647/server_vault/Projects/drwork/x12dev_drwork/rst/x12dev_drwork_rst/PXL/1273/p1273d6_Tchks.rs.rca 1.3 Sat Sep 27 23:32:00 2014 kperrey Experimental $

// $Log: p1273d6_Tchks.rs.rca $
// 
...
#ifndef _P1273D6_TCHKS_RS_
#define _P1273D6_TCHKS_RS_
...
SoftConnPwelltap:double = 0;
drValHash["SoftConnPwelltap"] = SoftConnPwelltap;
drHash["SoftConnPwelltap"] = "Pwelltap is soft-connected to pwell";
drErrGDSHash[xc(SoftConnPwelltap)] = { 1, 2001 };
...
// do the check
#ifndef _drTapCheck
drSoftCheck_(xc(SoftConnWelltap), softcheckWelltapStamped, (NWELL or (NWELLESDDRAWN not WELLRES_ID) or DEEPNWELL), stamped_trace_WellTapsoft_connect); 
#endif
...
// add subtaps back into connectivity
softcheckSubtapStamped = stamp(softcheckSubtap, ppickup, trace_soft_connect, stamped_SubTapsoft_connect);

// do the check
#ifndef _drTapCheck
#ifndef _drRCextract
drSoftCheck_(xc(SoftConnSubtap), softcheckSubtapStamped, pwell, stamped_SubTapsoft_connect); 
#else
   drSoftCheck_(xc(SoftConnSubtap), softcheckSubtapStamped, pwell, stamped_SubTapsoft_connect); 
   drSoftCheck_(xc(SoftConnSubtap), softcheckSubtapStamped, pwell_ext, stamped_SubTapsoft_connect); 
#endif
#endif

// get pwelltaps connected to devices
softcheckPwelltapNS = net_select(
      connect_sequence = stamped_trace_soft_connect_t3,
      layer_groups = { "layerSet1" => { pwelltap }}, 
      net_type = ALL,
      connected_to_any = {nsd_softconStamped, psd_softconStamped, save_gate_softconStamped,
                 vdsource, vddrain, vsdsrc,
                 anode, anode_nbjt, anode_nbjt_esd,
      ...
      output_from_layers = {pwelltap}
);
softcheckPwelltapID = pwelltap and DIODEID;
softcheckPwelltap = softcheckPwelltapNS or softcheckPwelltapID;

// add pwelltaps back into connectivity
softcheckPwelltapStamped = stamp(softcheckPwelltap, pwelltap, trace_soft_connect, stamped_PwellTapsoft_connect);

// do the check
#ifndef _drTapCheck
#ifndef _drRCextract
drSoftCheck_(xc(SoftConnPwelltap), softcheckPwelltapStamped, pwell, stamped_PwellTapsoft_connect); 
#else
   drSoftCheck_(xc(SoftConnPwelltap), softcheckPwelltapStamped, pwell, stamped_PwellTapsoft_connect); 
   drSoftCheck_(xc(SoftConnSubtap), softcheckSubtapStamped, pwell_ext, stamped_SubTapsoft_connect); 
#endif
...
#ifdef _drDEBUG
drPassthruStack.push_back({softcheckPwelltapStamped, {1008,201} }); // 
...
if (false) {
   ...
   // this is a soft_check assume shunt in TM1 or M12

   #if _drPROCESS == 6
      waiver_mim61_via12 = copy_by_cells(VIA12, cells = {"d8xtochnce12ressmall", "d8xtochnce12reslarge"} , depth = CELL_LEVEL);
      ce1v12 = (VIA12 and CE1) not waiver_mim61_via12;
      ce1_soft_connect = connect(
         connect_items = {
            {layers = {C4BEMIB, tm1nores}, by_layer = TV1 },
            {layers = {tm1nores, metal12nores, ce1v12}, by_layer = VIA12 } }
      );
      drSoftCheck_(xc(MIM_61), ce1v12, CE1, ce1_soft_connect); 
   
      ce2v12 = (VIA12 and CE2) not waiver_mim61_via12;
      ce2_soft_connect = connect(
         connect_items = {
            {layers = {C4BEMIB, tm1nores}, by_layer = TV1 },
            {layers = {tm1nores, metal12nores, ce2v12}, by_layer = VIA12 } }
      );
      drSoftCheck_(xc(MIM_61), ce2v12, CE2, ce2_soft_connect); 
   ...
   // connectivity for mim construction checks                        
      ...
      mim_construction_connect = connect(
         connect_items = {
            {layers = {C4BEMIB, tm1nores}, by_layer = TV1 },
            {layers = {tm1nores, metal12nores, CE12Chk, CE22Chk}, by_layer = VIA12 },
      ...
      ce1FloatingAll = CE12Chk not_interacting VIA12;
      ce2FloatingAll = CE22Chk not_interacting VIA12;
   #else
      mim_construction_connect = connect(
         connect_items = {
            {layers = {C4BEMIB, tm1nores}, by_layer = TV1 },
...
txt_std_checks = text_net (
   ...
   text_layer_items = {
      ...
      {layer = pwell_ext, text_layer = SUBISOTEXT_txt}  // text for substrate
...
pwelltap_vx_98 = pwelltap not osTSV;
...
trace_vx_98 = incremental_connect(
   ...
   connect_items = {
      ...
      {layers = {pwelltap_vx_98, ppickup, dnpdnw_anode}, by_layer = pwell },
      {layers = {pwelltap_vx_98, dnpdnw_anode}, by_layer = pwell_dnw },
      {layers = {nwell_nores, dnpdnw_cathode, dpndnw_cathode}, by_layer = DEEPNWELL },
      {layers = {ppickup_vx_98, nac_anode, dpndnw_anode, djnw_anode}, by_layer = __drsubstrate },
...
Error_Std_Checks @= {
   ...
   @ "DEEPNWELL interacting with 2 or more subiso regions";
   DEEPNWELL interacting [count = >=2] PWELL_SUBISO;

   @ "NODE can not connect both PWELL and SUBSTRATE";
   net_select(
      connect_sequence = txt_trace_netlist_connect,
      net_type = ALL,
      connected_to_all = {pwell_dnw, __drsubstrate},
      output_from_layers = {pwell_dnw}
   );  
}
